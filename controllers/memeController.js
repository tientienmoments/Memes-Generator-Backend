...
memeController.createMeme = async (req, res, next) => {
  try {
    ...
    meme.texts = texts && texts.length >= 1 ? texts.map((text) => JSON.parse(text)) : [];
    // Put text on image
...
memeController.getMemes = (req, res, next) => {
  try {
    ...
    return utilsHelper.sendResponse(res, 200, true, { memes, totalPages }, null, "Get memes successful");
  } catch (err) {
    next(err);
  }
};
